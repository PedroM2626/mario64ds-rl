...
class ImpalaCNN(nn.Module):
    """
    Feature extractor based on the IMPALA architecture with residual blocks.
    Pure PyTorch module designed for Tianshou Rainbow and related agents.
    """
    ...
    def forward(self, observations: torch.Tensor, state=None, info={}):
        ...
        # Ensure CHW format for PyTorch Conv2D.
        if observations.dim() == 5 and observations.shape[-1] == 1:
            # FrameStackObservation output: (Batch, Stack, H, W, 1) -> Squeeze -> (Batch, Stack, H, W)
        ...
        # Scale to [0, 1] if input is uint8 (0-255)
...
class TianshouNatureCNN(nn.Module):
    """NatureCNN (Mnih et al., 2015) adapted for Tianshou.

    Identical architecture to standard SB3 CnnPolicy for a controlled, fair
    comparison between Rainbow+NatureCNN and PPO+NatureCNN. Accepts HWC/5D
    tensor formats matching ImpalaCNN and exposes output_dim.
    """
...
class RainbowNet(nn.Module):
    """
    Combines visual feature extractor with Dueling Network heads and NoisyLinear layers.
    Returns probability distributions (batch_size, action_shape, num_atoms) for C51 / RainbowPolicy.
    """
    def __init__(self, feature_net, action_shape, num_atoms=51, noisy_std=0.1):
        ...
        # Value head (V) - estimates state value distribution
        ...
        # Advantage head (A) - estimates action advantage distributions
    ...
    def forward(self, obs, state=None, info={}):
        ...
        # Tianshou C51 policy expects probability distributions, not raw logits.
        # Softmax is applied across the atom dimension (dim=-1)
